...
cfg_cinc_license={{cfg.cinc_license.acceptance}}
cfg_cinc_license="${cfg_cinc_license:-undefined}"

<<<<<<< HEAD
if [ "${cfg_chef_license}" = "undefined" ]; then
  cfg_chef_license_cmd=""
=======
if [ "${cfg_cinc_license}" == "undefined" ]; then
  cfg_cinc_license_cmd=""
>>>>>>> cinc
else
  cfg_cinc_license_cmd="--cinc-license ${cfg_cinc_license}"
fi

cinc_client_cmd()
{
  # Disables the Shellcheck that requires a double quote around a variable
  # This only applies to the cfg_cinc_license_cmd because putting quotes around the variable
  # causes the Chef Client to think that the policyfile is be overriden which is unsupported
  # and causes the cinc run to fail.
  # shellcheck disable=SC2086
  "{{pkgPathFor "${scaffold_cinc_client}"}}/bin/cinc-client" -z -l "${cfg_log_level}" -c {{pkg.svc_config_path}}/client-config.rb -j {{pkg.svc_config_path}}/attributes.json --once --no-fork --run-lock-timeout "${cfg_run_lock_timeout}" $cfg_cinc_license_cmd
}
